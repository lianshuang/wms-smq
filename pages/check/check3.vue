	...
	<view>
		...
		<view class="operation">
			...
			<view class="scanner">
				...
				<input class="scanner-input uni-input" cursor-spacing="10" type="text" placeholder="扫描SKU NO." focus @confirm="nextStep()" v-model="formData.code" />
			</view>
			<!-- 底栏操作按钮 -->
			...
			validateForm() {
				return new Promise((resolve, reject) => {
					if (!this.formData.code) {
						...
						resolve(false)
					}else if(this.formData.code!=this.requestData.sku_code){
						uni.showToast({
							title: '扫描的SKU与目标SKU不一致',
							icon: 'none',
							mask: true,
							duration: 2000
						});
